"use client";

import { Question } from "@/lib/types";
import { CheckCircle, XCircle } from "lucide-react";
import { useState, useEffect } from "react";

interface QuestionCardProps {
  question: Question;
  selectedIndex: number | null;
  submitted: boolean;
  onSelect: (index: number) => void;
  showExplanation: boolean;
}

export default function QuestionCard({
  question,
  selectedIndex,
  submitted,
  onSelect,
  showExplanation,
}: QuestionCardProps) {
  const letters = ["A", "B", "C", "D"];
  
  // State to hold the shuffled indices
  const [shuffledIndices, setShuffledIndices] = useState<number[]>([]);
  
  // Clean option text (remove prefix like "A) " or "B. ")
  const cleanOption = (text: string) => {
    return text.replace(/^[A-D][\)\.]\s*/, "");
  };

  // Shuffle options on mount or when question changes
  useEffect(() => {
    // Create an array of indices [0, 1, 2, 3]
    const indices = [0, 1, 2, 3];
    
    // Fisher-Yates shuffle
    for (let i = indices.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [indices[i], indices[j]] = [indices[j], indices[i]];
    }
    
    setShuffledIndices(indices);
  }, [question.id]); // Re-shuffle when question changes

  // If not yet shuffled, don't render to avoid hydration mismatch
  if (shuffledIndices.length === 0) {
    return <div className="space-y-4 animate-pulse h-64 bg-[#1a1a2e] rounded-lg"></div>;
  }

  return (
    <div className="space-y-4">
      {shuffledIndices.map((originalIdx, displayIdx) => {
        const option = question.options[originalIdx];
        const isSelected = selectedIndex === originalIdx;
        const isCorrect = originalIdx === question.correctIndex;
        const isWrong = isSelected && !isCorrect && submitted;

        let buttonClass = "bg-[#2d3561] border border-[#4a5a8a] text-[#E2E8F0] hover:bg-[#3a4578]";

        if (submitted) {
          if (isCorrect) {
            buttonClass = "bg-[#10B981]/20 border border-[#10B981] text-[#E2E8F0]";
          } else if (isWrong) {
            buttonClass = "bg-[#EF4444]/20 border border-[#EF4444] text-[#E2E8F0]";
          }
        } else if (isSelected) {
          buttonClass = "bg-[#3B82F6] border border-[#60A5FA] text-white";
        }

        return (
          <button
            key={originalIdx}
            onClick={() => !submitted && onSelect(originalIdx)}
            disabled={submitted}
            className={`w-full p-4 rounded-lg text-left transition-smooth flex items-start gap-4 cursor-pointer ${buttonClass}`}
          >
            <span className="font-bold text-lg min-w-fit">{letters[displayIdx]}.</span>
            <span className="flex-1">{cleanOption(option)}</span>
            {submitted && isCorrect && <CheckCircle className="text-[#10B981] flex-shrink-0" size={20} />}
            {submitted && isWrong && <XCircle className="text-[#EF4444] flex-shrink-0" size={20} />}
          </button>
        );
      })}
    </div>
  );
}
